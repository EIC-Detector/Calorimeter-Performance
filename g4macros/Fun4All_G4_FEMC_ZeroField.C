int
Fun4All_G4_FEMC_ZeroField(
			   int nEvents = 10,
			   const char * outputFile = "G4FEMC_e-_p10_eta-2_detailed_TEST.root",
			   const char * ptype = "e-",
			   float ppmin = 10,
			   float ppmax = 10,
			   float petamin = 2,
			   ...
			   )
{
  ...
  cout << "Run Fun4All_G4_FEMC_ZeroField with... " << endl;
  ...
  //----------------------
  G4CaloShowerAnalysis* showerAnalysis = new G4CaloShowerAnalysis( "G4Shower_FEMC" , "G4Shower_FEMC.root" );
  showerAnalysis->AddG4HitNode("G4HIT_FEMC");
  showerAnalysis->SetStoreESum( true , 2001 , -0.005 , 20.005 );

  se->registerSubsystem(showerAnalysis);

  G4CaloShowerAnalysis* showerAnalysisAbsorber = new G4CaloShowerAnalysis( "G4Shower_Abs_FEMC" , "G4Shower_Abs_FEMC.root" );
  showerAnalysisAbsorber->AddG4HitNode("G4HIT_ABSORBER_FEMC");
  showerAnalysisAbsorber->SetStoreESum( true , 2001 , -0.005 , 20.005 );

  se->registerSubsystem(showerAnalysisAbsorber);
  ...
  tower_FEMC->Detector("FEMC", calotowerid::FEMC);
  ...
  //----------------------
  G4CaloTowerAnalysis* towerAnalysis = new G4CaloTowerAnalysis( "TowerAna_FEMC" , "TowerAna_FEMC.root" );
  towerAnalysis->AddTowerNode("TOWER_FEMC");
  towerAnalysis->SetStoreESum( true , 2001 , -0.005 , 20.005 );
